...
all_trips$started_at <- format(all_trips$started_at, format = "%Y-%m-%d %H:%M:%OS")
all_trips$ended_at <- format(all_trips$ended_at, format = "%Y-%m-%d %H:%M:%OS")
...
all_trips$started_at <- as.POSIXct(all_trips$started_at, format = "%Y-%m-%d %H:%M:%OS")
all_trips$ended_at <- as.POSIXct(all_trips$ended_at, format = "%Y-%m-%d %H:%M:%OS")
...
aggregate(all_trips_v2$ride_length ~ all_trips_v2$member_casual + all_trips_v2$day_of_week, FUN = mean) # the average ride time by each day for members vs casual users
